...
export default function StatsSection({ stats, period }) {
  ...
  const statItems = [
    ...
    { label: 'REPLIES', value: stats.total_replies || 0 },
  ];
    ...
    <section className="py-12">
      ...
      >
        <h2 className="text-3xl font-bold text-white mb-2">
          Stats for <span className="gradient-text">{periodLabels[period]}</span>
        </h2>
        <p className="text-white/40 text-sm">Community engagement metrics</p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {statItems.map((item, index) => (
          ...
          >
            ...
            <p className="text-3xl md:text-4xl font-bold gradient-text font-mono">{formatNumber(item.value)}</p>
          </motion.div>
        ))}
      </div>
        ...
        >
          <div className="glass-card p-5">
            <div className="flex items-center gap-2 mb-4">
              <Sparkles className="w-4 h-4 text-rs-red" />
              <span className="text-rs-raspberry font-semibold text-sm">Most Viewed Tweet</span>
              <span className="text-white/30 text-xs font-mono ml-auto">{formatNumber(stats.most_viewed_tweet.views_count)} views</span>
            </div>
